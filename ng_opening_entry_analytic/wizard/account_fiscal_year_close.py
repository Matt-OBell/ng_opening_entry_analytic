...
class account_fiscalyear_close(osv.osv_memory):
    ...
    _columns = {
        'is_include_analytic' : fields.boolean('Include Analytic?'),
        'analytic_account_id' : fields.many2one('account.analytic.account', 'Default Analytic Account For Type Balances')
    }
    
    _defaults ={
        'is_include_analytic' : True
    }
    
    def data_save(self, cr, uid, ids, context=None):
        is_include_analytic = self.browse(cr, uid, ids, context=context)[0].is_include_analytic
        #probuse check if is_include_analytic is False on Generate Opening Entries Wizard then Super Call
        if not is_include_analytic:
            return super(account_fiscalyear_close, self).data_save(cr, uid, ids, context=context)
        #probuse check if is_include_analytic is True on Generate Opening Entries Wizard then carry forward the analytic account id 
        else:
            ...
            if account_ids:
                cr.execute('''
                    INSERT INTO account_move_line (
                         ...
                         quantity, product_id, company_id,analytic_account_id)
                      (SELECT name, create_uid, create_date, write_uid, write_date,
                         statement_id, %s,currency_id, date_maturity, partner_id,
                         blocked, credit, 'draft', debit, ref, account_id,
                         %s, (%s) AS date, %s, amount_currency, quantity, product_id, company_id, analytic_account_id
                       FROM account_move_line
                       WHERE account_id IN %s
                         AND ''' + query_line + '''
                         AND reconcile_id IS NULL)''', (new_journal.id, period.id, period.date_start, move_id, tuple(account_ids),))
                #probuse added 'analytic_account_id' in above insert query to pass analytic account id
                ...
                cr.execute('''
                    INSERT INTO account_move_line (
                      ...
                      (SELECT
                         ...
                         b.quantity, b.product_id, b.company_id, b.analytic_account_id
            ...
            query_1st_part = """
                    INSERT INTO account_move_line (
                         ...
                         account_id, currency_id, amount_currency, company_id, state, analytic_account_id) VALUES
            """
            #probuse added 'analytic_account_id' in query_1st_part insert query
            query_2nd_part = ""
            query_2nd_part_args = []
            for account in obj_acc_account.browse(cr, uid, account_ids, context={'fiscalyear': fy_id}):
                ...
                if not currency_obj.is_zero(cr, uid, company_currency_id, abs(account.balance)):
                    if query_2nd_part:
                        query_2nd_part += ','
                    query_2nd_part += "(%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s)"#probuse added %s 
                    query_2nd_part_args += (account.balance > 0 and account.balance or 0.0,
                           ...
                           'draft',
                           data[0].analytic_account_id and data[0].analytic_account_id.id or False#probuse
                           )
            if query_2nd_part:
